...
DECLARE
  ...
  game_id INTEGER;
  worker_game_id_offset INTEGER := 10000; -- Will be replaced with 1000, 2000, etc. for workers 1-5
BEGIN
  -- Worker 1: Use base game ID and base usernames
  ...
  audience1_id := get_worker_user_id('TestAudience1', 1);
  ...
  INSERT INTO game_participants (game_id, user_id, role, status)
  VALUES (game_id, get_worker_user_id('TestPlayer1', 1), 'player', 'active');
  ...
  VALUES (
    game_id,
    get_worker_user_id('TestPlayer1', 1),
  ...
  WHERE c.game_id = 339  -- Use literal game ID to avoid ambiguity
    AND c.user_id = get_worker_user_id('TestPlayer1', 1)
