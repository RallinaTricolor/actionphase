...
DECLARE
  ...
  game_id INTEGER;
  worker_game_id_offset INTEGER := 30000; -- Will be replaced with 1000, 2000, etc. for workers 1-5
BEGIN
  -- Worker 3: Use base game ID and base usernames
  ...
  audience1_id := get_worker_user_id('TestAudience1', 3);
  ...
  -- Add TestAudience1 as co-GM for NPC messaging tests
  -- Add TestAudience2 as audience (candidate for co-GM promotion in other tests)
  ...
  INSERT INTO game_participants (game_id, user_id, role, status)
  VALUES (game_id, get_worker_user_id('TestPlayer1', 3), 'player', 'active');
  ...
  VALUES (
    game_id,
    get_worker_user_id('TestPlayer1', 3),
  ...
  WHERE c.game_id = 339  -- Use literal game ID to avoid ambiguity
    AND c.user_id = get_worker_user_id('TestPlayer1', 3)
    AND c.character_type = 'player_character'
    AND gp.phase_number = 1
  LIMIT 1;

  RAISE NOTICE 'Co-GM Management fixture created: Game #% with 2 audience members, 1 player, 4 characters (1 player, 1 GM NPC, 2 unassigned NPCs), 1 action', game_id;
