...
LEFT JOIN game_participants gp ON gp.game_id = g.id AND gp.user_id = $1 AND gp.removed_at IS NULL
LEFT JOIN characters c ON c.game_id = g.id AND c.user_id = $1
WHERE g.gm_user_id = $1 OR (gp.user_id = $1 AND gp.removed_at IS NULL)
ORDER BY
    CASE g.state
