# Character Sheet Detail

The character sheet is your character's record — it holds their public description, private notes, abilities, skills, inventory, and currency. Open it by clicking your character's name in the **People** tab.

## Tabs

The sheet has four tabs:

- **Public Profile** — Character description visible to all players
- **Private Notes** — Your private notes, visible only to you, the GM, and audience members
- **Abilities & Skills** — Special powers and trained skills (GM-controlled)
- **Inventory** — Equipment, items, and currency/resources (GM-controlled)

The **Public Profile** tab is always visible to everyone. The other three tabs are only visible to the character's player, the GM, and audience members. In a completed game, all participants can view the full sheet.

## Public Profile

The **Character Description** field holds your character's public-facing information — appearance, personality, backstory, and anything else other players can know. Supports Markdown. Edit it by clicking **Edit** on the field.

## Private Notes

A single **Private Notes & Secrets** field for anything you don't want other players to see — motivations, secrets, things your character knows. Supports Markdown. Visible only to you, the GM, and audience members.

## Abilities & Skills

The **Abilities & Skills** tab has two sub-tabs:

### Abilities

Each ability has:
- **Name** — What the ability is called
- **Type** — Either **Learned** or **Innate**
- **Description** — What the ability does

### Skills

Each skill has:
- **Name** — The skill name
- **Level** — Numeric proficiency level
- **Category** — Optional grouping (e.g., "Combat", "Social")
- **Description** — What the skill covers

## Inventory

The **Inventory** tab has two sub-tabs:

### Items

Each item has:
- **Name** — Item name
- **Quantity** — How many the character has
- **Category** — Optional grouping (e.g., "Weapon", "Consumable")
- **Value** — Optional monetary value
- **Weight** — Optional weight
- **Description** — Notes about the item

### Currency/Resources

Each currency entry has:
- **Type** — The currency or resource name (e.g., "Gold", "Credits", "XP")
- **Amount** — The current quantity
- **Description** — Optional notes

Currency is private — only the character's player, the GM, and audience members can see it.

## What Players Can Edit

Players can edit their **Public Profile** and **Private Notes** directly. **Abilities, Skills, Items, and Currency are GM-controlled** — only the GM can add, edit, or remove them. The GM can update these sections at any time, or bundle changes into published action results.

## Avatar

Click the camera icon on the character avatar to upload a new image. Click the trash icon to remove it.

## Renaming a Character

Players can rename their own character by clicking the pencil icon next to the name. GMs can rename any character.
